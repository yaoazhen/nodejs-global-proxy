...
function setProxy(proxyUri, enableLog = false) {
    ...
    // Generate shell script for easy sourcing
    const shellScript = generateShellScript(uri, injectorPath, enableLog);
    
    // Show instructions for shell integration
    console.log('');
    log('blue', 'To apply these settings to your current shell session, run:');
    log('yellow', 'source .np-proxy-env.sh');
    console.log('');
    log('blue', 'Or manually export the variables:');
    ...
    console.log('');
    log('blue', 'For permanent setup, add these lines to your ~/.bashrc or ~/.zshrc.');
}

function generateShellScript(proxyUri, injectorPath, enableLog) {
    const scriptContent = `#!/bin/bash
# Generated by np - Node.js Process Proxy Manager
# Run: source .np-proxy-env.sh

export NODEJS_GLOBAL_SOCKS5_PROXY="${proxyUri}"
export NODE_OPTIONS="--require ${injectorPath}"${enableLog ? '\nexport NODEJS_PROXY_DEBUG_LOG="true"' : ''}

echo "✅ Node.js SOCKS5 proxy environment variables set!"
echo "Proxy: \\$NODEJS_GLOBAL_SOCKS5_PROXY"${enableLog ? '\necho "Debug logging: ENABLED"' : ''}
echo ""
echo "Test with: node test-proxy.js (if available)"
`;

    // Write the script to current directory synchronously
    import('fs').then(fs => {
        fs.writeFileSync('.np-proxy-env.sh', scriptContent);
    }).catch(() => {
        // Fallback - use dynamic import in a different way
        try {
            const fs = require('fs');
            fs.writeFileSync('.np-proxy-env.sh', scriptContent);
        } catch (e) {
            console.error('Failed to write .np-proxy-env.sh:', e.message);
        }
    });
...
function unsetProxy() {
    ...
    // Generate unset shell script
    const unsetScript = `#!/bin/bash
# Generated by np - Node.js Process Proxy Manager
# Run: source .np-proxy-env.sh
...
echo "✅ Node.js SOCKS5 proxy environment variables unset!"
echo "Proxy is now DISABLED for new Node.js processes."
`;

    // Write the unset script
    import('fs').then(fs => {
        fs.writeFileSync('.np-proxy-env.sh', unsetScript);
    }).catch(() => {
        const fs = require('fs');
        fs.writeFileSync('.np-proxy-env.sh', unsetScript);
    });
    ...
    log('yellow', 'source .np-proxy-env.sh');
    console.log('');
    log('blue', 'Or manually unset the variables:');
